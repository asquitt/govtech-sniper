...
from app.api.routes.reviews.helpers import (
...
class TestReviewPacketActionRecommendation:
    def test_closed_status(self):
        result = review_packet_action_recommendation("critical", "closed")
        assert "No immediate action" in result

    def test_verified_status(self):
        result = review_packet_action_recommendation("major", "verified")
        assert "No immediate action" in result

    def test_rejected_status(self):
        result = review_packet_action_recommendation("minor", "rejected")
        assert "No immediate action" in result

    def test_critical_severity(self):
        result = review_packet_action_recommendation("critical", "open")
        assert "immediate owner" in result.lower() or "patch" in result.lower()

    def test_major_severity(self):
        result = review_packet_action_recommendation("major", "open")
        assert "draft cycle" in result.lower()

    def test_minor_severity(self):
        result = review_packet_action_recommendation("minor", "open")
        assert "cleanup" in result.lower() or "quality" in result.lower()

    def test_suggestion_severity(self):
        result = review_packet_action_recommendation("suggestion", "open")
...
class TestReviewPacketExitCriteria:
    ...
    def test_unknown_type_defaults_to_gold(self):
        ...
        criteria = review_packet_exit_criteria("unknown", 50.0, 1, 3)
        assert len(criteria) == 4

    def test_criteria_contain_current_values(self):
        criteria = review_packet_exit_criteria("pink", 72.5, 2, 5)

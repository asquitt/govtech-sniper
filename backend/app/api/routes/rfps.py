...
async def list_rfps(
    ...
    status: Optional[RFPStatus] = Query(None, description="Filter by status"),
...
async def diff_rfp_snapshots(
    rfp_id: int = Path(..., description="RFP ID"),
    from_snapshot_id: Optional[int] = Query(
...
async def get_rfp_stats(
    ...
    qualified_result = await session.execute(
        select(func.count(RFP.id))
        .where(RFP.user_id == resolved_user_id, RFP.is_qualified == True)
    )
    qualified = qualified_result.scalar()
    
    disqualified_result = await session.execute(
        select(func.count(RFP.id))
        .where(RFP.user_id == resolved_user_id, RFP.is_qualified == False)
    )

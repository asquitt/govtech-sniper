...
def upgrade() -> None:
    ...
    op.add_column(
        "teaming_partners", sa.Column("naics_codes", sa.JSON(), nullable=True, server_default="[]")
    )
    op.add_column(
        "teaming_partners", sa.Column("set_asides", sa.JSON(), nullable=True, server_default="[]")
    )
    op.add_column(
        "teaming_partners", sa.Column("capabilities", sa.JSON(), nullable=True, server_default="[]")
    )
    op.add_column("teaming_partners", sa.Column("clearance_level", sa.String(50), nullable=True))
    op.add_column(
        "teaming_partners", sa.Column("past_performance_summary", sa.Text(), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "from_user_id", sa.Integer(), sa.ForeignKey("users.id"), nullable=False, index=True
        ),
        sa.Column(
            "to_partner_id",
            sa.Integer(),

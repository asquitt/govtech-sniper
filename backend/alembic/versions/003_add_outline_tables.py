...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("proposal_id", sa.Integer(), sa.ForeignKey("proposals.id"), nullable=False, unique=True, index=True),
    ...
    op.create_table(
        "outline_sections",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("outline_id", sa.Integer(), sa.ForeignKey("proposal_outlines.id"), nullable=False, index=True),
